...
const CopyWindow = ({
  ...
  useEffect(() => {
    if (data != undefined && permissions.view && bizComponentData !== null) {
      if (data.length > 0) {
  ...
  const fetchMainGrid = async (filters: any) => {
    ...
    const parameters: Iparameters = {
      procedureName: "P_SA_A9001W_Sub2_Q",
      pageNumber: 1,
      pageSize: PAGE_SIZE,
      parameters: {
        "@p_work_type": "Q",
  ...
  const selectData = (selectedData: any) => {
    ...
    setParaData((prev) => ({
      ...
      custcd: mainDataResult.data[0].custcd,
      custnm: mainDataResult.data[0].custnm,
  ...
  const para: Iparameters = {
    procedureName: "P_SA_A9001W_Sub2_S",
    pageNumber: 0,
    pageSize: 0,
    parameters: {
