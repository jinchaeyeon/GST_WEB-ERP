...
const CopyWindow = ({
  ...
  const fetchMainGrid = async () => {
    ...
    if (data.isSuccess == true) {
      ...
      const totalRowCnt5 = data.tables[6].RowCount;
      const totalRowCnt6 = data.tables[7].RowCount;
      const totalRowCnt7 = data.tables[8].RowCount;
      const rows = data.tables[0].Rows;
      const rows2 = data.tables[1].Rows;
      const rows3 = data.tables[2].Rows;
      const rows4 = data.tables[3].Rows;
      const rows5 = data.tables[6].Rows;
      const rows6 = data.tables[7].Rows;
      const rows7 = data.tables[8].Rows;
      setMainDataResult(() => {
        return {
          data: rows5,
      ...
      if (totalRowCnt3 > 0) {
        setInformation((prev) => ({
          ...
          recoverqty_ex: rows3[0].recoverqty,
  ...
  const InputChange = (e: any) => {
    ...
    if (name == "experimentqty_tk") {
      ...
      setInformation((prev) => ({
        ...prev,
        [name]: value,
        totgeomche_base: value * prev.geomcheprodqty_base,
      }));
    } else if (name == "geomcheprodqty_base") {
      setInformation((prev) => ({
        ...prev,
        [name]: value,
        totgeomche_base: value * prev.geomcheqty_base,
      }));
