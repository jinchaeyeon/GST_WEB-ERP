...
const AC_A0070W: React.FC = () => {
  ...
  UseBizComponent("L_AC001, L_AC061, L_BA002, L_BA020, L_BA028", setBizComponentData);
  ...
  const [applyMinWidthGridMaster, setApplyMinWidthGridMaster] = React.useState(false);
  const [applyMinWidthGridItem, setApplyMinWidthGridItem] = React.useState(false);
  ...
  useEffect(() => {
    if (customOptionData !== null 
      ...
      const acntses = bizComponent?.data.Rows?.find(
        (item: any) => item.extra_field1 <= today && today <= item.extra_field2
      )?.sub_code ?? "";

      setFilters((prev) => ({
        ...prev,
        location: defaultOption?.find((item: any) => item.id === "location")?.valueCode ?? "",
        position: defaultOption?.find((item: any) => item.id === "position")?.valueCode ?? "",
        acntses: acntses,
        isSearch: true,
      }));
    }
  ...
  useEffect(() => {
    ...
    if (mainDataResult.total > 0) {
      ...
      if (selectedValue
        ...
        // find_row_value
        let selectedRowIndex = mainDataResult.data.findIndex((row:any) => 
          listIdGetter(row) == selectedValue
        );

        // 첫번째 행 선택하기
        setSelectedState({ [listIdGetter(mainDataResult.data[selectedRowIndex])]: true });
  ...
  const Retrieve = async (workType: string) => {
    ...
    if (data.isSuccess === true) {
      ...
      if (workType == "Q") {
        setPage({
          skip: filters.pgNum == 1 || filters.pgNum == 0
            ? 0 
        ...
        if (totalRowCnt > 0) {
          ...
          if (!!filters.find_row_value) {
            selectedRow = rows.find((row:any) => 
                listIdGetter(row) == filters.find_row_value
              );
          }
  ...
  const mainFooterCell = (props: GridFooterCellProps) => {
    if (mainDataResult.total > 0
      && numberField.includes(props.field ?? "")) {
      let sum = 0;
  ...
  const onCarryClick = () => {
    ...
    const acntsesnm = bizComponentData?.find(
      (item: any) => item.bizComponentId == "L_AC061" // 회기
    )?.data?.Rows?.find(
      (item: any) => item.sub_code == filters.acntses
    )?.code_name ?? "";
  ...
  const ExecuteSave = async (workType: string) => {
    ...
    if (isSuccess) {
      setMainDataResult(process([], mainDataState));
      setFilters((prev) => ({ ...prev, pgNum: 1, isSearch: true, pgGap: 0, find_row_value: data.returnString }));
    } 
    else {
      console.log("[오류 발생]");
      console.log(data);
  ...
  useEffect(() => {
    if (customOptionData != null) {
      gridMaster.current = document.getElementById("grdMaster");
      gridItem.current = document.getElementById("grdItem");

      window.addEventListener("resize", handleResize);

      customOptionData.menuCustomColumnOptions["grdMaster"].map((item: TColumn) =>
        item.width !== undefined
          ? (minWidthGridMaster.current += item.width)
          : minWidthGridMaster.current
      );

      customOptionData.menuCustomColumnOptions["grdItem"].map(
        (item: TColumn) =>
          item.width !== undefined
            ? (minWidthGridItem.current += item.width)
            : minWidthGridItem.current
      );
  ...
  const setWidth = (Name: string, minWidth: number | undefined) => {
    ...
    if (Name == "grdMaster" && !applyMinWidthGridMaster) {
      width +=
          ((currentWidthGridMaster - minWidthGridMaster.current) /
            customOptionData.menuCustomColumnOptions[Name].length);
    } 

    else if (Name == "grdItem" && !applyMinWidthGridItem) {
      width += 
        ((currentWidthGridItem - minWidthGridItem.current) /
            customOptionData.menuCustomColumnOptions[Name].length);
    };
                  ...
                  (item: any, idx: number) =>
                    item.sortOrder !== -1 && (
                      <GridColumn
                        ...
                        title={item.caption}
                        width={setWidth("grdMaster", item.width)}
                        cell={
                        ...
                        footerCell={
                          item.sortOrder === 0 ? mainTotalFooterCell : mainFooterCell
                        }
                      />
                    )
                )}
                  ...
                  (item: any, idx: number) =>
                    item.sortOrder !== -1 && (
                      <GridColumn
                        ...
                        field={item.fieldName}
                        title={item.caption}
                        width={setWidth("grdItem", item.width)}
                        className={
