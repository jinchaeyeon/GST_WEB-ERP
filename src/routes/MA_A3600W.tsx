...
const MA_A3600W: React.FC = () => {
  ...
  const exportExcel = () => {
    if (_export !== null && _export !== undefined) {
      ...
      optionsGridOne.sheets[0].title = "출고상세자료";
      optionsGridOne.sheets[1].title = "입고상세자료";
      optionsGridOne.sheets[2].title = "처리영역";
  ...
  const fetchMainGrid2 = async (filters2: any) => {
    ...
    const parameters: Iparameters = {
      ...
      parameters: {
        ...
        "@p_lotnum": filters2.lotnum,
        "@p_recdt": convertDateToStr(filters2.recdt),
    ...
    if (data.isSuccess == true) {
      ...
      if (filters2.find_row_value !== "") {
        ...
        if (gridRef2.current) {
          const findRowIndex = rows.findIndex(
            (row: any) => row.recfind == filters2.find_row_value
          );
      ...
      if (totalRowCnt > 0) {
        const selectedRow =
          filters2.find_row_value == ""
            ? rows[0]
            : rows.find((row: any) => row.recfind == filters2.find_row_value);
  ...
  const resetAllGrid = () => {
    ...
    setTempResult(process([], tempState));
  };
  ...
  const createColumn = () => {
    ...
    array.push(
      <GridColumn field={"load_place"} title={"창고"} width="120px" />
    );
    array.push(
      <GridColumn
        field={"now_qty"}
  ...
  const createColumn2 = () => {
    const array = [];
    array.push(
      <GridColumn field={"itemcd_af"} title={"품목코드"} width="150px" />
    );
    array.push(
      <GridColumn field={"itemnm_af"} title={"품목명"} width="150px" />
    );
    array.push(
      <GridColumn field={"itemacnt_af"} title={"품목계정"} width="120px" />
    );
    array.push(<GridColumn field={"insiz_af"} title={"규격"} width="120px" />);
    array.push(
      <GridColumn
        field={"doqty"}
        title={"처리량"}
    ...
    array.push(
      <GridColumn field={"qtyunit_af"} title={"단위"} width="120px" />
    );
    array.push(<GridColumn field={"remark"} title={"비고"} width="120px" />);
    return array;
  };
