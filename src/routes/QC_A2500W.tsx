...
const QC_A2500W: React.FC = () => {
  ...
  UseCustomOption("QC_A2500W", setCustomOptionData);
  ...
  useEffect(() => {
    if (bizComponentData !== null) {
      setCausedListData(getBizCom(bizComponentData, "L_QC001"));
      ...
      if (filters.find_row_value !== "") {
        ...
        if (gridRef.current) {
          const findRowIndex = rows.findIndex(
            (row: any) =>
              row.badnum + "-" + row.badseq ==
              filters.find_row_value.split("|")[0]
          );
    ...
    if (data.isSuccess == true) {
      ...
      if (detailFilters.find_row_value !== "") {
        ...
        if (gridRef2.current) {
          const findRowIndex = rows.findIndex(
            (row: any) =>
              row.badnum + "-" + row.badseq + "|" + row.datnum ==
    ...
    if (data.isSuccess == true) {
      ...
      if (isLastDataDeleted) {
        ...
        setDetailFilters((prev) => ({
          ...prev,
          find_row_value:
            detailDataResult.data[findRowIndex < 1 ? 1 : findRowIndex - 1]
              .stdnum,
