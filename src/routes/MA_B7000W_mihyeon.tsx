...
import { gridList } from "../store/columns/MA_B7000W_C_mihyeon";
...
const MA_B7000_mihyeon: React.FC = () => {
  ...
  UseCustomOption("MA_B7000W_mihyeon", setCustomOptionData);
  ...
  const [itemplaceQueryStr, setItemplaceQueryStr] = React.useState([
    COM_CODE_DEFAULT_VALUE,
  ]);
  ...
  useEffect(() => {
    if (bizComponentData !== null) {
      ...
      setItemplaceQueryStr(getBizCom(bizComponentData, "L_LOADPLACE"));
    }
  }, [bizComponentData]);
  ...
  const fetchMainGrid = async (filters: any) => {
    ...
    const parameters: Iparameters = {
      ...
      parameters: {
        ...
        "@p_yyyymm": convertDateToStr(filters.ymdyyyy).slice(0, 4), // 날자는 연도만 사용
    ...
    if (data.isSuccess == true) {
      ...
      // 조회된 데이터에 Rows에 데이터가 있어서 편하게 미리 뺌
  ...
  UseMessages("MA_B7000W_mihyeon", setMessagesData);
          ...
          <Grid
            ...
            data={process(
              mainDataResult.data.map((row) => ({
                ...
                itemplace: itemplaceQueryStr.find(
...
export default MA_B7000_mihyeon;
