...
import DetailWindow from "../components/Windows/QC_A6050W_Window";
import { useApi } from "../hooks/api";
import { IItemData } from "../hooks/interfaces";
import { isLoading } from "../store/atoms";
import { gridList } from "../store/columns/QC_A6050W_C";
import { Iparameters, TColumn, TGrid, TPermissions } from "../store/types";

const DATA_ITEM_KEY = "num";
const dateField = ["qcdt"];
      ...
      if (filters.find_row_value !== "") {
        ...
        if (gridRef.current) {
          const findRowIndex = rows.findIndex(
            (row: any) => row.qcnum == filters.find_row_value
          );
      ...
      if (totalRowCnt > 0) {
        const selectedRow =
          filters.find_row_value == ""
            ? rows[0]
            : rows.find((row: any) => row.qcnum == filters.find_row_value);
    ...
    if (data.isSuccess == true) {
      ...
      if (isLastDataDeleted) {
        ...
        setFilters((prev) => ({
          ...prev,
          find_row_value:
            mainDataResult.data[findRowIndex < 1 ? 1 : findRowIndex - 1].qcnum,
