...
const CustomComboBoxCell = (props: GridCellProps) => {
  ...
  UseBizComponent(
    "L_SYS005,L_BA002,L_BA028,L_dptcd_001,L_HU005,L_BA410,L_BA001",
    setBizComponentData
  );
  ...
  const onSaveClick = async () => {
    ...
    try {
      dataItem.forEach((item: any) => {
        if (!item.user_id) {
          throw findMessage(messagesData, "CR_A0010W_002");
        }

        if (!item.user_name) {
          throw findMessage(messagesData, "CR_A0010W_003");
        }

        if (!item.password) {
          throw findMessage(messagesData, "CR_A0010W_005");
        }
        if (!item.temp) {
          throw findMessage(messagesData, "CR_A0010W_006");
        }
        // if (!item.user_category) {
        //   throw findMessage(messagesData, "CR_A0010W_007");
        // }
      });
    } catch (e) {
      alert(e);
    ...
    try {
      ...
      for (const item of dataItem) {
        ...
        if (password !== temp) {
          throw findMessage(messagesData, "CR_A0010W_004");
        }
            ...
            <div className={isMobile ? "leading_PDA_custom" : ""}>
              <GridContainer style={{ paddingBottom: "15px", height: "100%", width: "100%" }}>
                ...
                >
                  <Grid
                    style={{
                      height: isMobile ? `${deviceHeight * 0.65}px` : "77.8vh",
